...
static int http_server_push_jpg(void* /*http*/, http_session_t* session, const char* /*method*/, const char* path)
{
	char header[1024];

	http_server_set_header(session, "Content-Type", "multipart/x-mixed-replace; boundary=frame");
	http_server_set_header(session, "Cache-Control", "no-cache");
	http_server_set_content_type(session, "image/png");
	http_server_sendfile(session, "./1.jpg", NULL, NULL);

	http_server_set_content_type(session, "image/png");
	return http_server_sendfile(session, "./2.jpg", NULL, NULL);

	http_server_set_status_code(session, 404, NULL);
	return http_server_send(session, "", 0, NULL, NULL);
}

static int http_server_onwebsocket(void* param, struct http_websocket_t* ws, const char* path, const char* subprotocols, void** wsparam)
{
...
void http_server_test(const char* ip, int port)
{
	size_t num = 0;
	...
	http_server_addroute("/", http_server_push_jpg);
...
{
    if (argc < 3) {
        printf("try: ./test 127.0.0.1 8080\r\n");
        exit(0);
    }

    http_server_test(argv[1], atoi(argv[2]));
    return 0;
}
